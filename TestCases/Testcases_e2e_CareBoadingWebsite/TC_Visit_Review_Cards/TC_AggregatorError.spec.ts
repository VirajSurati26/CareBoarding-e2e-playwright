...
import { Aggregatorcard } from "@/pageObjects/Visit_Review_In_Visits_Module/Visit_Review_AggregatorCard";
...
test('Visit Review appears in Visits module', async ({ page }) => {
  await loginAndSelectEntity(page);
    ...
    //Verify the select the "Aggregator card" card in Visit review page
    await visitReviewPage.Click_Aggregatorcard();
    await page.waitForTimeout(5000)
    
    //-------------------------------------------------------------------------------------
    //--------------------Verification of "Aggregator Error" card Data----------------------------------
    //-------------------------------------------------------------------------------------
